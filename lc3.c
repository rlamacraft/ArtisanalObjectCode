...
int main(int argc, const char* argv[]) {
  ...
  while (running) {
    ...
    if(debug) {
      printf("INSTR: %04x, PC: %04x, [%04x, %04x, %04x, %04x, %04x, %04x, %04x, %04x, %04x] \n", instr, reg[R_PC], reg[R_R0], reg[R_R1], reg[R_R2], reg[R_R3], reg[R_R4], reg[R_R5], reg[R_R6], reg[R_R7], reg[R_COND]);
    }
    ...
    switch (op) {
      ...
      case OP_JMP:
        {
          ...
          uint16_t mem_flag = (instr >> 11) & 0x1;
          if(mem_flag) {
            reg[R_PC] = mem_read(reg[baseR]--);
          } else {
            reg[R_PC] = reg[baseR];
          }
        }
      ...
      case OP_JSR:
        {
          mem_write(++reg[R_R7], reg[R_PC]);
          /* printf("[%x] = %x", reg[R_R7], reg[R_PC]); */
      ...
      case OP_LDR:
        {
          ...
          reg[r0] = mem_read(reg[baseReg] + offset);
          update_flags(r0);
        }
        break;
      case OP_LEA:
